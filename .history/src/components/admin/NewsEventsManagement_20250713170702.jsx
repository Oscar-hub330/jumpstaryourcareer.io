...
const NewsEventsManagement = () => {
  ...
  const [previewType, setPreviewType] = useState("template");
    ...
    <Box sx={{ p: 4 }}>
      ...
      <Card sx={{ p: 3, mb: 4, backgroundColor: "#fff8f0", borderRadius: 3, boxShadow: 3 }}>
        <CardContent>
                  ...
                  {section.images.map((img, imgIdx) => (
                    <Box key={imgIdx} sx={{ position: "relative" }}>
                      <img
                        ...
                        style={{ width: 120, height: 80, objectFit: "cover", borderRadius: 6, boxShadow: "0 2px 8px rgba(0,0,0,0.2)" }}
                      />
                      <IconButton
                        size="small"
                        color="error"
          ...
          {previewing && previewType === "pdf" && pdfFiles.length > 0 && (
            <Box sx={{ mt: 4 }}>
              ...
              <Box sx={{ display: "flex", gap: 2, mb: 2 }}>
                {pdfFiles.map((pdf, idx) => (
                  <Button
                    key={idx}
                    variant={activePdfIndex === idx ? "contained" : "outlined"}
              ...
              <iframe
                src={pdfFiles[activePdfIndex].url}
                title={`PDF Preview ${activePdfIndex + 1}`}
                width="100%"
                height="600px"
          ...
          {previewing && previewType === "template" && (
            <Box sx={{ mt: 4, backgroundColor: "#fefefe", p: 3, border: "1px solid #ddd", borderRadius: 2 }}>
              <Typography variant="h5" sx={{ mb: 2, color: "#fea434" }}>Template Preview</Typography>
              {newsletterTemplates[templateIndex].map((section, idx) => (
                <Box key={idx} sx={{ mb: 4 }}>
                  <Typography variant="h6">{section.title}</Typography>
                  <Typography variant="body2" color="textSecondary">{section.date}</Typography>
                  <Typography align={section.alignment} sx={{ mt: 1 }}>{section.paragraph}</Typography>
