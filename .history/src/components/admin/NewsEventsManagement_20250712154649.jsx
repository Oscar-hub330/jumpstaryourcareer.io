...
const NewsEventsManagement = () => {
  ...
  // Validate current template before publish (simple)
  const validateTemplate = () => {
    const sections = newsletterTemplates[templateIndex];
    if (!sections.length) return false;
    for (const sec of sections) {
      if (!sec.title.trim() || !sec.date || !sec.paragraph.trim()) return false;
    }
    return true;
  };
      ...
      <Card sx={{ p: 3, mb: 4, backgroundColor: "#fff8f0", borderRadius: 3, boxShadow: 3 }}>
        <CardContent>
          ...
          {newsletterTemplates[templateIndex].map((section, secIdx) => (
            <Card key={secIdx} sx={{ mb: 3, p: 2, backgroundColor: "#fff" }}>
          ...
          <Box sx={{ mt: 4, display: "flex", gap: 2 }}>
            <Button
              ...
              onClick={() => {
                if (!validateTemplate()) {
                  alert("Please fill in title, date, and paragraph in all sections.");
                  return;
                }
                setPreviewing(true);
              }}
